...
from config import getenv_float, getenv_str, parse_timestamp_to_utc_iso
...
def _audit_path() -> str:
    return getenv_str("AUDIT_LOG", "/var/log/audit/audit.log")


def _iso_from_audit_key(audit_key: str) -> str:
    # audit key format commonly: "<epoch>.<msec>:<id>"
    ts_part = audit_key.split(":", 1)[0].strip()
    return parse_timestamp_to_utc_iso(ts_part)
...
def _flush_record(
    ...
    for path in paths:
        yield {
            ...
            "source": "auditd",
            "audit_id": key,
            "timestamp": _iso_from_audit_key(key),
            "pid": int(pid),
            "executable": exe,
            "file": path,
            "action": "access",
...
def _iter_tail_lines(path: str) -> Generator[str, None, None]:
    ...
    if shutil.which("tail") is None:
        LOGGER.warning("tail command not found; falling back to python file follower for %s", path)
        yield from _iter_lines_python_follow(path)
        return
    ...
    try:
        ...
        LOGGER.error("tail command not found; cannot stream audit log")
        return
...
def _iter_lines_python_follow(path: str) -> Generator[str, None, None]:
    poll = max(0.1, getenv_float("DLDS_AUDIT_POLL", 0.2))
    ...
    try:
        while True:
            ...
            st = os.stat(path)
            if inode != st.st_ino or handle is None:
...
def process_event_stream() -> Generator[Dict[str, Any], None, None]:
    path = _audit_path()
    LOGGER.info("Process monitor initialized audit_log=%s", path)

    while True:
        while not os.path.exists(path):
            LOGGER.debug("auditd log not found at %s; retrying", path)
            time.sleep(2.0)
        ...
        for line in _iter_tail_lines(path):
            ...
            if not m:
                continue
            LOGGER.debug("Read auditd line successfully")
            key = m.group(1)
            ...
            if typ == "SYSCALL":
                ...
                LOGGER.debug("Flushing auditd event key=%s", key)
