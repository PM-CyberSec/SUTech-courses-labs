...
class Correlator:
    ...
    def _emit(self, record: Dict[str, Any]) -> None:
        ...
        ai_res = self._ai_engine.predict(record)
        record["ai_label"] = ai_res.get("ai_label", "benign")
        record["confidence"] = ai_res.get("confidence", 0.0)
        record["anomaly_score"] = ai_res.get("anomaly_score", 0.0)
        record["ai_reason"] = ai_res.get("ai_reason", "")
        record["ai_evidence"] = ai_res.get("ai_evidence", [])
        record["model_version"] = ai_res.get("model_version", "")
        record["correlation_summary"] = ai_res.get("correlation_summary", "")

        line = json.dumps(record, ensure_ascii=False)
        LOGGER.info("Pipeline emitted event type=%s severity=%s ai_label=%s anomaly_score=%s", record.get("type"), record.get("severity"), record.get("ai_label"), record.get("anomaly_score"))
    ...
    def _post_http(self, record: Dict[str, Any]) -> None:
        ...
        try:
            ...
            response = self._http.post(self._api_url, data=raw_body, headers=headers, timeout=detection_api_timeout())
            ...
            with sqlite3.connect(str(sqlite_path)) as conn:
                ...
                cur.execute(
                    ...
                    INSERT INTO dlds_events (
                        ...
                        ai_label, confidence, anomaly_score, ai_reason, ai_evidence, model_version, correlation_summary
                    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                    """,
                    (
                        ...
                        json.dumps(record.get("ai_evidence", [])),
                        record.get("model_version"),
                        record.get("correlation_summary"),
                    ),
                )
                conn.commit()
        except Exception:
