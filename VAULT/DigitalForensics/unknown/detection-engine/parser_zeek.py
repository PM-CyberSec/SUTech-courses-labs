...
from config import parse_timestamp_to_utc_iso
...
def parse_conn_data_line(field_names: List[str], line: str) -> Optional[Dict[str, Any]]:
    ...
    if "local_orig" in row:
        local_orig = _bool_field(_get(row, "local_orig", default="T"))
    else:
        # Some conn.log variants omit local_orig; default to initiator=local.
        local_orig = True
    ...
    return {
        "ts_raw": ts_s,
        "timestamp": parse_timestamp_to_utc_iso(ts_s),
        "src_ip": orig_h,
        "src_port": orig_port,
        "dst_ip": resp_h,
        "dst_port": resp_port,
