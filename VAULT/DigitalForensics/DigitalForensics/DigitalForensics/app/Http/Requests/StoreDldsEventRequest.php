<?php

declare(strict_types=1);

namespace App\Http\Requests;

use Carbon\CarbonImmutable;
use Illuminate\Foundation\Http\FormRequest;

class StoreDldsEventRequest extends FormRequest
{
    public function authorize(): bool
    {
        return true;
    }

    /**
     * @return array<string, mixed>
     */
    public function rules(): array
    {
        return [
            'event_time' => ['nullable', 'date'],
            'timestamp' => ['nullable', 'date'],
            'event_type_id' => ['nullable', 'integer', 'exists:event_types,id'],
            'type' => ['nullable', 'string', 'max:100'],
            'pid' => ['nullable', 'integer', 'min:0'],
            'process_id' => ['nullable', 'integer', 'exists:process_catalog,id'],
            'process_name' => ['nullable', 'string', 'max:255'],
            'file_path' => ['nullable', 'string', 'max:255'],
            'file' => ['nullable', 'string', 'max:255'],
            'src_ip' => ['nullable', 'ip'],
            'dst_ip' => ['nullable', 'ip'],
            'src_port' => ['nullable', 'integer', 'min:0', 'max:65535'],
            'dst_port' => ['nullable', 'integer', 'min:0', 'max:65535'],
            'bytes_sent' => ['nullable', 'integer', 'min:0'],
            'alert_type_id' => ['nullable', 'integer', 'exists:alert_types,id'],
            'alert_type' => ['nullable', 'string', 'max:150'],
            'severity_id' => ['nullable', 'integer', 'exists:severity_levels,id'],
            'severity' => ['nullable', 'in:LOW,MEDIUM,HIGH,CRITICAL'],
            'description' => ['nullable', 'string', 'max:65535'],
            'ai_label' => ['nullable', 'string', 'in:benign,suspicious,malicious'],
            'confidence' => ['nullable', 'numeric', 'min:0', 'max:1'],
            'anomaly_score' => ['nullable', 'numeric', 'min:0', 'max:1'],
            'ai_reason' => ['nullable', 'string'],
            'ai_evidence' => ['nullable', 'array'],
            'model_version' => ['nullable', 'string'],
        ];
    }

    protected function prepareForValidation(): void
    {
        $alert = $this->input('alert');
        if (! is_array($alert)) {
            $alert = [];
        }

        $eventTime = $this->input('event_time')
            ?: ($this->input('timestamp') ?: ($this->input('time') ?: $this->input('ts')));
        $eventTime = $this->normalizeTimestampValue($eventTime);

        $severity = $this->input('severity');
        if (is_numeric($severity)) {
            $severity = $this->severityFromNumeric((int) $severity);
        }

        if (($severity === null || $severity === '') && ! $this->filled('severity_id')) {
            $severityNum = $this->input('severity_num')
                ?? $this->input('priority')
                ?? ($alert['severity'] ?? null);

            if (is_numeric($severityNum)) {
                $severity = $this->severityFromNumeric((int) $severityNum);
            }
        }

        $type = $this->input('type') ?: $this->input('event_type');

        $this->merge([
            'event_time' => $eventTime,
            'timestamp' => $eventTime,
            'type' => $type ?: null,
            'severity' => $severity ?: null,
            'process_name' => $this->input('process_name')
                ?: ($this->input('process') ?: ($this->input('executable') ?: $this->input('program'))),
            'file_path' => $this->input('file_path')
                ?: ($this->input('file') ?: $this->input('path')),
            'src_ip' => $this->input('src_ip')
                ?: ($this->input('source_ip')
                    ?: (data_get($this->input('src'), 'ip')
                        ?: (data_get($this->input('source'), 'ip') ?: data_get($this->input('source'), 'address')))),
            'dst_ip' => $this->input('dst_ip')
                ?: ($this->input('dest_ip')
                    ?: ($this->input('destination_ip')
                        ?: (data_get($this->input('dst'), 'ip')
                            ?: (data_get($this->input('dest'), 'ip')
                                ?: (data_get($this->input('destination'), 'ip') ?: data_get($this->input('destination'), 'address')))))),
            'src_port' => $this->input('src_port')
                ?? ($this->input('source_port')
                    ?? (data_get($this->input('src'), 'port')
                        ?? (data_get($this->input('source'), 'port') ?? data_get($this->input('source'), 'p')))),
            'dst_port' => $this->input('dst_port')
                ?? ($this->input('dest_port')
                    ?? ($this->input('destination_port')
                        ?? (data_get($this->input('dst'), 'port')
                            ?? (data_get($this->input('dest'), 'port')
                                ?? (data_get($this->input('destination'), 'port') ?? data_get($this->input('destination'), 'p')))))),
            'alert_type' => $this->input('alert_type')
                ?: ($this->input('alert_signature')
                    ?: ($this->input('signature')
                        ?: (($alert['signature'] ?? null)
                            ?: ($this->input('category') ?: ($alert['category'] ?? null))))),
        ]);

        foreach ([
            'event_time',
            'timestamp',
            'type',
            'process_name',
            'file_path',
            'file',
            'src_ip',
            'dst_ip',
            'alert_type',
            'description',
        ] as $key) {
            if ($this->isNullMarker($this->input($key))) {
                $this->merge([$key => null]);
            }
        }

        if (! $this->filled('type') && ! $this->filled('event_type_id')) {
            $this->merge(['type' => 'alert']);
        }

        if (! $this->filled('severity') && ! $this->filled('severity_id')) {
            $this->merge(['severity' => 'LOW']);
        }

        if ($this->filled('type')) {
            $this->merge(['type' => strtolower((string) $this->input('type'))]);
        }

        if ($this->filled('severity')) {
            $this->merge(['severity' => strtoupper((string) $this->input('severity'))]);
        }

        if ($this->filled('ai_label')) {
            $this->merge(['ai_label' => strtolower((string) $this->input('ai_label'))]);
        }
    }

    private function isNullMarker(mixed $value): bool
    {
        if ($value === null) {
            return true;
        }

        $normalized = strtolower(trim((string) $value));

        return in_array($normalized, ['', '-', '--', 'null', '(null)', 'n/a'], true);
    }

    private function severityFromNumeric(int $severity): string
    {
        if ($severity <= 1) {
            return 'CRITICAL';
        }

        if ($severity === 2) {
            return 'HIGH';
        }

        if ($severity === 3) {
            return 'MEDIUM';
        }

        return 'LOW';
    }

    private function normalizeTimestampValue(mixed $value): mixed
    {
        if ($value === null) {
            return null;
        }

        if (is_int($value) || is_float($value)) {
            return CarbonImmutable::createFromTimestampUTC((int) floor((float) $value))->toIso8601String();
        }

        $raw = trim((string) $value);
        if ($raw === '') {
            return null;
        }

        if (is_numeric($raw)) {
            return CarbonImmutable::createFromTimestampUTC((int) floor((float) $raw))->toIso8601String();
        }

        return $raw;
    }
}
