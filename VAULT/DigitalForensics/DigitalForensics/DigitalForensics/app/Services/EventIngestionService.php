...
class EventIngestionService
{
    public function __construct(
        ...
        private readonly EventClassificationGuard $classificationGuard,
    ) {}
    ...
    public function ingest(array $payload): array
    {
        ...
        $normalized = [
            ...
            'src_ip' => $srcIp,
            'src_port' => $this->asPort($this->firstPresent($payload, ['src_port', 'source_port', 'src.port', 'source.port']) ?? 0),
            'dst_ip' => $dstIp,
            'dst_port' => $this->asPort($this->firstPresent($payload, ['dst_port', 'dest_port', 'destination_port', 'dst.port', 'dest.port', 'destination.port']) ?? 0),
            ...
            'ai_label' => $this->nullableString($this->firstPresent($payload, ['ai_label'])),
            'confidence' => (float) ($this->firstPresent($payload, ['confidence']) ?? 0.0),
            'anomaly_score' => (float) ($this->firstPresent($payload, ['anomaly_score']) ?? 0.0),
            'ai_reason' => $this->nullableString($this->firstPresent($payload, ['ai_reason'])),
            'ai_evidence' => $this->payloadGet($payload, 'ai_evidence'),
            'model_version' => $this->nullableString($this->firstPresent($payload, ['model_version'])),
        ];

        $normalized = $this->applyClassificationGuard(
            $normalized,
            type: $detectedType,
            severity: $severityInput ?? 'LOW',
            alertType: $alertType,
        );

        $hash = $this->hashEvent($normalized);

        $qualityWarnings = $this->qualityWarnings($normalized, $detectedType, $hash);
        if ($qualityWarnings !== []) {
            Log::warning('DLDS event payload missing normalized fields', [
                'event_type' => 'dlds.ingest.data_quality',
                'missing_fields' => $qualityWarnings,
                'detected_type' => $detectedType,
                'event_hash' => $hash,
                'payload_keys' => array_values(array_keys($payload)),
                'source' => $this->nullableString($this->firstPresent($payload, ['event.source', 'source_name', 'sensor'])),
            ]);
        }
        ...
        if ($event->wasRecentlyCreated) {
            try {
                ...
                Log::warning('DLDS realtime broadcast dispatch failed', [
                    'event_type' => 'dlds.broadcast.dispatch_failed',
                    'event_id' => $event->id,
     ...
     */
    private function applyClassificationGuard(
        array $normalized,
        string $type,
        string|int $severity,
        ?string $alertType,
    ): array {
        $guarded = $this->classificationGuard->apply([
            'type' => $type,
            'severity' => is_int($severity) ? $this->normalizeSuricataSeverity($severity) : strtoupper((string) $severity),
            'alert_type' => $alertType,
            'description' => $normalized['description'] ?? null,
            'ai_label' => $normalized['ai_label'] ?? null,
            'confidence' => $normalized['confidence'] ?? 0.0,
            'anomaly_score' => $normalized['anomaly_score'] ?? 0.0,
            'ai_reason' => $normalized['ai_reason'] ?? null,
        ]);

        $normalized['ai_label'] = $this->nullableString($guarded['ai_label'] ?? null);
        $normalized['confidence'] = (float) ($guarded['confidence'] ?? 0.0);
        $normalized['anomaly_score'] = (float) ($guarded['anomaly_score'] ?? 0.0);
        $normalized['ai_reason'] = $this->nullableString($guarded['ai_reason'] ?? null);
    ...
    private function detectType(array $payload): string
    {
        ...
        $srcIp = $this->nullableString($this->firstPresent($payload, ['src_ip', 'source_ip', 'src.ip', 'source.ip', 'source.address']));
        $dstIp = $this->nullableString($this->firstPresent($payload, ['dst_ip', 'dest_ip', 'destination_ip', 'dst.ip', 'dest.ip', 'destination.ip', 'destination.address']));
        $srcPort = $this->asPort($this->firstPresent($payload, ['src_port', 'source_port', 'src.port', 'source.port']) ?? 0);
        $dstPort = $this->asPort($this->firstPresent($payload, ['dst_port', 'dest_port', 'destination_port', 'dst.port', 'dest.port', 'destination.port']) ?? 0);
     ...
     * @return string[]
     */
    private function qualityWarnings(array $normalized, string $detectedType, string $hash): array
    {
        $missing = [];

        if (($normalized['event_type_id'] ?? null) === null) {
            $missing[] = 'event_type_id';
        }

        if (($normalized['severity_id'] ?? null) === null) {
            $missing[] = 'severity_id';
        }
        ...
        if ($this->nullableString($normalized['description'] ?? null) === null && $detectedType !== 'test') {
            $missing[] = 'description';
        }

        if ($detectedType === 'network') {
            if ($this->nullableString($normalized['src_ip'] ?? null) === null) {
                $missing[] = 'src_ip';
            }

            if ($this->nullableString($normalized['dst_ip'] ?? null) === null) {
                $missing[] = 'dst_ip';
            }
        }

        if ($detectedType === 'process'
            && (int) ($normalized['pid'] ?? 0) === 0
            && ($normalized['process_id'] ?? null) === null
            && $this->nullableString($normalized['file_path'] ?? null) === null
        ) {
