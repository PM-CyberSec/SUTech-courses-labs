...
create table address(
	street varchar(45),
	city varchar(45),
	user_id int primary key, -- user_id acts as PK and FK for 1-to-1 relationship [cite: 1, 2]
	foreign key (user_id) references users(user_id)
);

create table categories(
	category_id int primary key auto_increment,
	name varchar(50) -- Adjusted from 'category_name' to 'name' to match schema [cite: 1, 2]
);

create table user_likes (
	user_like_id int auto_increment primary key, -- Added primary key as per schema [cite: 1, 2]
	user_id int,
	category_id int,
	foreign key (user_id) references users(user_id),
...
create table products(
	...
	description text, -- Adjusted from 'product_description' to 'description' to match schema [cite: 1, 2]
	stock_quantity int,
...
create table payments(
	...
	amount decimal(10,2),
	method varchar(50),
	status varchar(50) -- Adjusted from 'payment_status' to 'status' to match schema [cite: 1, 2]
);

create table reviews(
	review_id int primary key auto_increment, -- Added auto_increment for primary key [cite: 1, 2]
	user_id int,
	foreign key (user_id) references users(user_id),
	product_id int,
	foreign key (product_id) references products(product_id),
